                        ...
                        <div class="border-b border-gray-200">
                            <form action="{{ route('post.update', ['post' => $posts->id]) }}" method="POST" class="space-y-6">
                                ...
                                <div class="flex justify-end">
                                    <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-orange-950 bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                        Save
                                    </button>
                                </div>
                            </form>
...
<div class="bg-orange-100 p-3 mb-2 card border-0 shadow-sm rounded-lg">
    <div class="flex items-center justify-content-between">
        ...
        <div class="flex items-center">
            ...
            <div>
                <a href="{{route('user.show', ['user' => $posts->user->id])}}">
                    <p class="font-semibold text-orange-950">{{ $posts->user->name }} {{ $posts->user->lastname }}</p>
                </a>
                <a href="{{route('user.show', ['user' => $posts->user->id])}}">
                    <p class="text-orange-950 text-sm">&#64;{{ $posts->user->username }}</p>
                    <p class="text-orange-950 text-sm">{{ $posts->created_at->format('M d, Y')  }}</p>
                </a>
            </div>
        </div>

        <div class="dropdown">
            <button class="dropdown-toggle text-orange-950" type="button" data-bs-toggle="dropdown" aria-expanded="false"> More
...
<p class="my-2 text-orange-950">{{ $posts->content }}</p>
    ...
    <img src="{{ asset($posts->image_url) }}" alt="Post Image" class="w-50 my-3 rounded-lg align-items-center">
    ...
    <!--Recipe-->
        ...
        <div class="bg-orange-200 card mb-3 border border-0 rounded-lg shadow-sm" style="max-width: 540px;">
        <a href="{{route('recipe.show', ['recipe' => $posts->recipe->id])}}">
            <div class="row g-0">
            
                <div class="col-md-4">
                    <img src="{{ asset($posts->recipe->image_url) }}" class="img-fluid rounded-start" alt="...">
                </div>
                <div class="col-md-8">
                    <div class="card-body">
                        <h5 class="card-title text-orange-950">{{ $posts->recipe->title }}</h5>
                        <p class="text-orange-800">By: {{ $posts->recipe->user->name }} {{ $posts->recipe->user->lastname }} (&#64;{{ $posts->recipe->user->username }})</p>
                    </div>
                </div>
    ...
    <!-- Bottoni Post -->
    @if(auth()->check())
    <div class="flex justify-between mt-3 px-4 border-t border-orange-900 py-3">
        ...
        <form action="{{ route('postlike.store') }}" method="POST">
            ...
            <button type="submit" class="flex items-center text-orange-950 text-sm">
        ...
        <button class="flex items-center text-orange-950 text-sm" type="button" data-bs-toggle="collapse" data-bs-target="#commentForm{{$posts->user->id}}" aria-expanded="false" aria-controls="commentForm{{$posts->user->id}}">
        ...
        @if($posts->user_id == Auth::id())
        @else
        <button class="flex items-center text-orange-950 text-sm" data-bs-toggle="modal" data-bs-target="#writePostModal" data-post-id="{{ $posts->id }}" data-post-content="{{ $posts->content }}" data-post-recipe-id="{{ $posts->recipe_id ?? '' }}" data-post-name="{{ $posts->user->name }}" data-post-lastname="{{ $posts->user->lastname }}" data-post-username="{{ $posts->username }}" data-post-image="{{ $posts->image_url }}">
        ...
        @endif
        <button class="flex items-center text-orange-950 text-sm">
    ...
    @else
    <div class="logFirst flex text-orange-950 hover:text-indigo-900 justify-between py-2 mt-3 px-4 rounded-lg">
    ...
    <div class="">
        ...
        @empty
        <p class="text-orange-950">No comments yet.</p>
        @endforelse
    </div>
    ...
    <form action="{{ route('postcomment.store') }}" method="POST" class="m-4 collapse" id="commentForm{{$posts->user->id}}" style="visibility: visible;">
        ...
        <div class="flex justify-end mt-2">
            <button type="submit" class="px-4 py-2 bg-orange-600 text-orange-950 rounded hover:bg-orange-700 focus:outline-none focus:bg-orange-800">
                Post Comment
            </button>
        </div>
    </form>
