    ...
    <div>
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg mt-6 p-6">
                <p class="font-semibold text-xl text-indigo-600">My Friends Requests</p>
                @if(session('success'))
                <p class="p-3 bg-indigo-600 text-white rounded-lg shadow-sm">Friend Request Accepted</p>
                @endif
                ...
                @endforeach
                @else
                <p>You don't have any friend request for now!</p>
                @endif
            </div>

            <div class="bg-white rounded-lg mt-6 p-6">
                <p class="font-semibold text-xl text-indigo-600">My Sent Friend Requests</p>
                    ...
                    <div class="flex items-center justify-content-between space-x-3 p-3">
                        <div class="d-flex align-items-center">
                            <a href="{{ route('user.show', ['user' => $friend->toUser->id]) }}">
                                <img src="{{ asset($friend->toUser->avatar) }}" alt="Profile picture of {{ $friend->toUser->name }}" class="w-12 h-12 rounded-full border-2 border-white shadow-sm">
                            </a>
                            <a href="{{ route('user.show', ['user' => $friend->toUser->id]) }}" class="text-decoration-none">
                                <h5 class="text-sm font-medium text-gray-900">{{ $friend->toUser->name }} {{ $friend->toUser->lastname }}</h5>
                                <p class="text-xs text-gray-500">&#64;{{ $friend->toUser->username }}</p>
                            </a>
                        </div>
                        <div class="d-flex ">

                            <button class="mt-1 rounded-full py-1 px-3 bg-indigo-500 text-white text-xs flex items-center hover:bg-indigo-600 focus:outline-none focus:bg-indigo-700 transition-colors">
                ...
                @endforeach
                @else
                <p>You didn't send any friend requests for now! Send some!</p>
                @endif
            </div>

            <div class="bg-white rounded-lg mt-6 p-6">
                <p class="font-semibold text-xl text-indigo-600">My Friends</p>
                @if($friends->count()>0)
                ...
                @endforeach
                @else
                <p>You don't have any friends for now! Add some!</p>
                @endif
            </div>
        </div>
    </div>
